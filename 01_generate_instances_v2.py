# * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * #
# Generates MILP instances of a certain type (setcover, cauctions, indset,      #
# ufacilities or mknapsack). Three directories are created: train, test and     #
# transfer.                                                                     #
# For mknapsack, four additional directories with MPS files (Gurobi format)    #
# are created alongside the standard LP directories.                            #
# Usage:                                                                        #
# python 01_generate_instances_v2.py <type> -s <seed>                           #
# python 01_generate_instances_v2.py mknapsack -s 2                             #
# * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * #
...
def generate_unsplittable_capacited_facility_location(random, filename, n_customers, n_facilities, ratio):
    """
...
def generate_mknapsack(number_of_items, number_of_knapsacks, filename, random,
    ...
    Returns weights, profits, capacities so the caller can also write MPS
    without consuming additional random state.
...
def write_mknapsack_mps(weights, profits, capacities, number_of_items, number_of_knapsacks, filename):
    """
    Write a Multiple Knapsack instance to MPS format for Gurobi.

    Uses data already sampled by generate_mknapsack so no additional random
    state is consumed.  Variable names match the companion LP file exactly.

    The MPS file uses OBJSENSE MAX so Gurobi maximises profit directly.
    All variables are declared binary via BV bounds.

    Parameters
    ----------
    weights : numpy array, shape (number_of_items,)
    profits : numpy array, shape (number_of_items,)
    capacities : numpy array, shape (number_of_knapsacks,)
    number_of_items : int
    number_of_knapsacks : int
    filename : str
        Path to the .mps file to write.
    """
    with open(filename, 'w') as file:
        file.write("NAME          mknapsack\n")

        # OBJSENSE MAX tells Gurobi to maximise the objective row directly.
        file.write("OBJSENSE\n")
        file.write("    MAX\n")
        ...
        # N  = free (objective)
        # L  = <= constraint
        # Ck (k=1..K)       : capacity constraint for knapsack k
        # C(K+i) (i=1..n)   : assignment constraint for item i
        file.write("ROWS\n")
        file.write(" N  OBJ\n")
        for k in range(number_of_knapsacks):
            file.write(f" L  C{k+1}\n")
        for i in range(number_of_items):
            file.write(f" L  C{number_of_knapsacks+i+1}\n")

        # ---- COLUMNS ----
        # Variable x{item + n_items*knapsack + 1} appears in:
        #   OBJ  : profit[item]
        #   Ck+1 : weight[item]   (capacity constraint of its knapsack)
        #   C(K+item+1) : 1       (assignment constraint of its item)
        #
        # Each MPS data line carries at most two (row, coeff) pairs;
        # split across two lines when needed.
        file.write("COLUMNS\n")
        file.write("    MARK0001    'MARKER'    'INTORG'\n")
        for k in range(number_of_knapsacks):
            for i in range(number_of_items):
                var = f"x{i + number_of_items*k + 1}"
                # line 1: OBJ coefficient + capacity row coefficient
                file.write(f"    {var}    OBJ    {profits[i]}    C{k+1}    {weights[i]}\n")
                # line 2: assignment row coefficient
                file.write(f"    {var}    C{number_of_knapsacks+i+1}    1\n")
        file.write("    MARK0002    'MARKER'    'INTEND'\n")

        # ---- RHS ----
        file.write("RHS\n")
        for k in range(number_of_knapsacks):
            file.write(f"    RHS    C{k+1}    {capacities[k]}\n")
        for i in range(number_of_items):
            file.write(f"    RHS    C{number_of_knapsacks+i+1}    1\n")

        # ---- BOUNDS ----
        # BV declares a binary (0/1) variable; no explicit lower/upper needed.
        file.write("BOUNDS\n")
        for k in range(number_of_knapsacks):
            for i in range(number_of_items):
                var = f"x{i + number_of_items*k + 1}"
                file.write(f" BV BND    {var}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['setcover', 'cauctions', 'indset', 'mknapsack', 'ufacilities'],
    )
    parser.add_argument(
        '-s', '--seed',
    ...
    if args.problem == 'setcover':
        ...
        for filename, ncs, nfs, r in zip(filenames, ncustomerss, nfacilitiess, ratios):
            print(f"  generating file {filename} ...")
            generate_unsplittable_capacited_facility_location(rng, filename, n_customers=ncs, n_facilities=nfs, ratio=r)
        ...
        filenames     = []   # LP paths
        mps_filenames = []   # MPS paths (Gurobi)
        nitemss       = []
        nknapsackss   = []
        ...
        mps_dir = f'data/instances/mknapsack/train_{number_of_items}_{number_of_knapsacks}_mps'
        print(f"{n} instances in {lp_dir} and {mps_dir}")
        os.makedirs(lp_dir)
        os.makedirs(mps_dir)
        filenames.extend([os.path.join(lp_dir,  f'instance_{i+1}.lp')  for i in range(n)])
        mps_filenames.extend([os.path.join(mps_dir, f'instance_{i+1}.mps') for i in range(n)])
        nitemss.extend([number_of_items] * n)
        ...
        mps_dir = f'data/instances/mknapsack/valid_{number_of_items}_{number_of_knapsacks}_mps'
        print(f"{n} instances in {lp_dir} and {mps_dir}")
        os.makedirs(lp_dir)
        os.makedirs(mps_dir)
        filenames.extend([os.path.join(lp_dir,  f'instance_{i+1}.lp')  for i in range(n)])
        ...
        mps_dir = f'data/instances/mknapsack/test_{number_of_items}_{number_of_knapsacks}_mps'
        print(f"{n} instances in {lp_dir} and {mps_dir}")
        os.makedirs(lp_dir)
        os.makedirs(mps_dir)
        filenames.extend([os.path.join(lp_dir,  f'instance_{i+1}.lp')  for i in range(n)])
        ...
        mps_dir = f'data/instances/mknapsack/transfer_{number_of_items}_{number_of_knapsacks}_mps'
        print(f"{n} instances in {lp_dir} and {mps_dir}")
        os.makedirs(lp_dir)
        os.makedirs(mps_dir)
        filenames.extend([os.path.join(lp_dir,  f'instance_{i+1}.lp')  for i in range(n)])
        ...
        # actually generate the instances
        for filename, mps_filename, nitems, nknapsacks in zip(filenames, mps_filenames, nitemss, nknapsackss):
            print(f"  generating file {filename} ...")
            weights, profits, capacities = generate_mknapsack(
                nitems, nknapsacks, filename, rng,
                min_range=10, max_range=20, scheme='subset-sum')
            write_mknapsack_mps(weights, profits, capacities, nitems, nknapsacks, mps_filename)
